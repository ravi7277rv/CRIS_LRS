...
const WeldFractureQuery = ({ layer, newMapObj, startLoaderForQueryBuilder, stopLoaderForQueryBuilder }) => {
    ...
    useEffect(() => {
        const bindWeldFractureDatas = async () => {
            try {
                const response = await fetch(`${globalUrlConfig.REACT_APP_API_BASE_URL}/getWeldFractureData`);
                if (!response.ok) {
    ...
    const handleFlawDetectedValue = (value) => (event) => {
        let selectedValue = value.value;
        let isChecked = event.target.checked;
        const { updatedValuesFromLayer, selectedValues } = updateValues(selectedValue, isChecked, flawDetectedValuesFromLayer, selectedFlawDetectedValues);
    ...
    const handleQueryData = async () => {
        ...
        let years = storedQueryObject.filter(f => f.id === id)[0].years;
        if (years.length === 0) {
        ...
        if (layer) {
            ...
            if (storedQueryObject) {
                const updatedQueryObject = storedQueryObject.map(item =>
                    item.id === 6 ? { ...item, query: finalQuery } : item
                );
...
export default WeldFractureQuery