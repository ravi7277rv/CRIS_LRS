...
const TrackQuallityQuery = ({ layer, newMapObj, startLoaderForQueryBuilder, stopLoaderForQueryBuilder }) => {
    ...
    useEffect(() => {
        const bindTrackQualityDatas = async () => {
            try {
                const response = await fetch(`${globalUrlConfig.REACT_APP_API_BASE_URL}/getTrackQualityData`);
                if (!response.ok) {
                    throw new Error('Failed to fetch data');
                }
                const result = await response.json();
                const sortedPerformance = result.data.performance.map(p => ({ value: p, checked: true }));
                setGradeOfSteelValuesFromLayer([{ value: "All", checked: true }, ...sortedPerformance]);
                setSelectedGradeOfSteelValues(result.data.performance.map(p => p));
            } catch (err) {
                console.log(err)
            } finally {
                //
