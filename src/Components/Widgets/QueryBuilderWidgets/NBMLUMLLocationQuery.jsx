...
const NBMLUMLLocationQuery = ({ layer, newMapObj, startLoaderForQueryBuilder, stopLoaderForQueryBuilder }) => {
    ...
    useEffect(() => {
        const bindUMLDatas = async () => {
            try {
                const response = await fetch(`${globalUrlConfig.REACT_APP_API_BASE_URL}/getUMLData`);
                if (!response.ok) {
                    throw new Error('Failed to fetch data');
                }
                const result = await response.json();
                const sortedParam = result.data.param.map(p => ({ value: p, checked: true }));
                setParameterValuesFromLayer([{ value: "All", checked: true }, ...sortedParam]);
                setSelectedParameterValues(result.data.param.map(p => p));
            } catch (err) {
                console.log(err)
            } finally {
                //
