...
const BridgeWaterLevelQuery = ({ layer, newMapObj, startLoaderForQueryBuilder, stopLoaderForQueryBuilder }) => {
    ...
    useEffect(() => {
        const bindBridgeORNDatas = async () => {
            try {
                const response = await fetch(`${globalUrlConfig.REACT_APP_API_BASE_URL}/getBridgeWaterLevelData`);
                if (!response.ok) {
                ...
                const sortedStructureType = result.data.structureType.map(st => ({ value: st, checked: true }));
                setStractureTypeValuesFromlayer([{ value: "All", checked: true }, ...sortedStructureType]);
                setSelectedStractureTypeValues(result.data.structureType.map(st => st));
            } catch (err) {
                console.log(err)
            } finally {
                //
