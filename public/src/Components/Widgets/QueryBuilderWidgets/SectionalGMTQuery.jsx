...
import { indiaExtent } from '../../../Templates/Template';
import QueryButtons from '../../Buttons/QueryButtons';

const SectionalGMTQuery = ({ layer, newMapObj, startLoaderForQueryBuilder, stopLoaderForQueryBuilder }) => {
    ...
    const handleQueryData = async () => {
        ...
        let finalQuery = "";
        if (selectedOperator && inputValue) {
            sectionalGmtQuery = `gmt ${selectedOperator} ${inputValue}`;
        }
        cQuery = storedQueryObject.filter(so => so.id === 16)[0].query;
        if (layer) {
            // finalQuery = sectionalGmtQuery;

            finalQuery = cQuery !== "" ? [sectionalGmtQuery, cQuery].filter(Boolean).join(" AND ") : sectionalGmtQuery;
            if (storedQueryObject) {
                const updatedQueryObject = storedQueryObject.map(item =>
                    item.id === 8 ? { ...item, query: finalQuery } : item
                );
    ...
    return (
        <div className="queryDiv" style={{ width: "370px" }}>
...
export default SectionalGMTQuery