...
const SectionalSpeedQuery = ({ layer, newMapObj, startLoaderForQueryBuilder, stopLoaderForQueryBuilder }) => {
    ...
    const handleQueryData = async () => {
        ...
        let finalQuery = "";
        if (selectedOperator && inputValue) {
            sectionalSpeedQuery = `speed ${selectedOperator} ${inputValue}`;
        }
        cQuery = storedQueryObject.filter(so => so.id === 16)[0].query;
        if (layer) {
            // finalQuery = sectionalSpeedQuery;
            finalQuery = cQuery !== "" ? [sectionalSpeedQuery, cQuery].filter(Boolean).join(" AND ") : sectionalSpeedQuery;
            if (storedQueryObject) {
                const updatedQueryObject = storedQueryObject.map(item =>
                    item.id === 9 ? { ...item, query: finalQuery } : item
                );
    ...
    return (
        <div className="queryDiv" style={{ width: "418px" }}>
...
export default SectionalSpeedQuery