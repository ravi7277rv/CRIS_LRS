...
const RailSectionQuery = ({ layer, newMapObj, startLoaderForQueryBuilder, stopLoaderForQueryBuilder }) => {
    ...
    useEffect(() => {
        ...
        const railSectionData = async () => {
            ...
            console.log(`These are the value of the Rail Section form the Rail Section : ${JSON.stringify(uniqueValues)}`);
            ...
            setRailSectionValuesFromLayer([{ value: "All", checked: true }, ...sortedValues]);
            setSelectedRailSectionValues(sortedValues.map(v => v.value));
        }
        railSectionData();
    }, []);
    //#endregion
    ...
    useEffect(() => {
        ...
        const gradeOfSteelData = async () => {
            ...
            console.log(`These are the value of the Grade of Steel form the Rail Section : ${JSON.stringify(uniqueValues)}`);
            const uniqueValuesArray = Array.from(uniqueValues).map(item => JSON.parse(item));
            const comparator = (a, b) => {
                if (typeof a.value === 'number' && typeof b.value === 'number') {
                    return a.value - b.value;
    ...
    useEffect(() => {
        ...
        const SpecailRouteData = async () => {
            ...
            const sortedValues = uniqueValuesArray.sort(comparator);
            setSpecailRouteValuesFromLayer([{ value: "All", checked: true }, ...sortedValues]);
            setSelectedSpecailRouteValues(sortedValues.map(v => v.value));
        }
        SpecailRouteData();
    }, []);
