...
import { baseTrackRenderer, nbmlOnTRCSimpleRenderer, railSectionClassBreaksRenderer } from '../../Templates/Renderer';
...
import { API_BASE_URL, deffinitionExpression, featureTable, junctionLayer, monsoonQuery, nbmlLayer, trackLayer } from '../../Templates/Template';
import { areAllValuesSelected, updateRailways, updateLegendForId4And6, addCircularBoundaries, updateLegendForId13, capitalizeFirstLetter } from '../Utils/Functions';
...
const SplitScreen = () => {
    ...
    const [initialMapObject, setInitialMapObject] = useState(null);
    ...
    const [allZoneValue, setAllZoneValue] = useState(['CR']);
    const [gLayerArray, setGLayerArray] = useState([])
    const queryObject = [
        { id: 1, query: "" },
        { id: 2, query: "" },
        { id: 3, query: "" },
    ...
    useEffect(() => {
        const getUserDataWithZoneAndDiv = async () => {
            ...
            try {
                let response = await fetch(API_BASE_URL + '/cris_getUser_Zone_Division', {
                    method: 'POST',
                    headers: {
    ...
    let sessionFinalQuery = JSON.parse(sessionStorage.getItem("finalQuery"));
    ...
    let groupLayer = new GroupLayer({
        title: "Grouped Feature Layers",
    });
    const handleSelectedZoneLayer = (item) => (event) => {
        debugger
        ...
        if (isChecked) {
            if (selectedValue === 'All') {
                ...
                setAllZoneValue(railwayNames.map(rn => rn.value));
                allValues = isChecked ? railwayNames.filter((v) => v.value !== "All").map((v) => v.value) : []
            } else {
                updatedValuesFromLayer = railwayNames.map((v) =>
                    v.value === selectedValue ? { ...v, isSelected: isChecked } : v
                );
                setAllZoneValue(prev => [...prev, selectedValue])
                allValues = [...railwayZone, selectedValue].filter((v) => v !== "All");
            }

        } else {
            setAllZoneValue(prev => prev.filter(f => f !== selectedValue))
            allValues = railwayZone.filter(f => f !== "All" && f !== selectedValue);
        }
        setRailwayNames(updatedValuesFromLayer)
        setRailwayZone(allValues)
    }

    useEffect(() => {
        allZoneValue.forEach(z => {
            const layer = new FeatureLayer({
                url: `https://mlinfomap.org/server/rest/services/maplayer_railways_detail_event_${z}/MapServer`,
                title: `${z}`,
                renderer: railSectionClassBreaksRenderer
            });
            setGLayerArray(prev => [...prev,layer])
        })
    },[allZoneValue]);
    useEffect(() => {
        gLayerArray.forEach(layer => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                let zoneData = jsonData.data.map(d => ({ ...d, isSelected: false }));
                let filteredZone;
                if (userData && userData.user !== 'portaladmin') {
                    filteredZone = zoneData.filter(zd => zd.value === userData.zone);
                    let selectedFilteredZone = filteredZone.map(fz => ({
                        ...fz,
                        isSelected: true
                    }));
                    setRailwayNames([]);
                    setRailwayNames([...selectedFilteredZone]);
                    setRailwayZone([])
                    setRailwayZone(filteredZone.map(z => z.value));
                } else {
                    setRailwayNames([]);
                    let updatedZoneData = zoneData.map(zone => {
                        if (zone.value === "CR") {
                            return { ...zone, isSelected: true };
                        }
                        return zone;
                    });

                    setRailwayNames([{ label: "All", value: "All", isSelected: false }, ...updatedZoneData]);
                    setRailwayZone([])
                    setRailwayZone(zoneData.filter(z => z.value === "CR").map(z => z.value));
                }
            } catch (error) {
                // setError(error);
            } finally {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(API_BASE_URL + '/getSection?zone=' + railwayZone.join() + '&division=' + railwayDivision.join());
                if (!response.ok) {
                    throw new Error('Failed to fetch data');
                }
                const jsonData = await response.json();
    ...
    useEffect(() => {
        const fetchEventDataStatus = async () => {
            try {
                const response = await fetch(API_BASE_URL + '/kpi_update_status');
                if (!response) {
    ...
    useEffect(() => {
        const loadInitialMap = async () => {
            ...
            map.addMany([trackFeatureLayer, junctionMapImagelayer, groupLayer]);
            const home = new Home({
                view: initialView,
            })
            const zoom = new Zoom({
        ...
        const loadMapForNewScreens = async () => {
            ...
            screens.forEach(async (screen) => {
                if (!prevScreensRef.current.some(prevScreen => prevScreen.id === screen.id)) {
                    ...
                    let userZone = userData.zone !== "" ? userData.zone : "";
                    let userDivision = userData.division !== "" ? userData.division : "";
                    userDefinitionExpression = `railway in ('${userZone}') AND division in ('${userDivision}')`
                    let userQuery = userData.user === "portaladmin" ? "" : userDefinitionExpression;
                    ...
                    if (screen.id === 2) {
                        // allZoneValue.forEach(z => {
                        //     const layer = new FeatureLayer({
                        //         url: `https://mlinfomap.org/server/rest/services/maplayer_railways_detail_event_CR/MapServer`,
                        //         title: `${z}`,
                        //         renderer: railSectionClassBreaksRenderer
                        //     });
                        //     setGLayerArray(prev => [...prev, layer])
                    ...
                    if (screen.id === 13) {
                        addCircularBoundaries(featurelayer, newView, newView.scale);
                    }
                    let nbmlFeatureLayer;
                    if (screen.id === 11) {
                        nbmlFeatureLayer = new FeatureLayer({
                            url: nbmlLayer,
                            // url: `${baseServiceUrlCris+"maplayer_trc_nbml_locations_event/MapServer"}`,
                    ...
                    function closeOtherExpands(exceptExpand) {
                        [expandLayerList, bgExpandPrint, bgExpand, queryData, yearExpand, assetCount].forEach(expand => {
                            if (expand !== exceptExpand) {
                                expand.collapse();
                            }
                        });
                    ...
                    window.addEventListener('click', (event) => {
                        const elements = [expandLayerList, bgExpandPrint, bgExpand, queryData, yearExpand, expandTableWidget, assetCount];
                    ...
                    ChartContainer.appendChild(renderChart());
                    assetCount.watch("expanded", (expanded) => {
                        if (expanded) {
                            try {
                                ...
                                ReactDOM.render(
                                    // <AssetCount
                                    //     newMapObj={newMapObj}
                                    //     screenAsset={assetCountOfScreens.filter(acs => acs.id === screen.id)}
                                    //     featureLayer={featurelayer}
                                    //     nbmlFeatureLayer={screen.id === 11 ? nbmlFeatureLayer : null}
                                    //     omsPeakFeatureLayer={screen.id === 12 ? omsPeakFeatureLayer : null}
                                    //     finalQuery={fQuery}
                    ...
                    const widgetsToMonitor = [yearExpand, expandTableWidget, bgExpandPrint, expandLayerList, queryData];
                    ...
                    // $(`.esri-ui .esri-component`).css("pointer-events", "none !important");
                    let layerView;
                    if (screen.id === 11) {
    ...
    useEffect(() => {
        const getFeaturesAndZoom = async () => {
            ...
            let finalQuery = "";
            let storedQueryObject = JSON.parse(sessionStorage.getItem("finalQuery"));
            const isAdmin = userData.user == 'portaladmin' ? true : false;
            // alert(userData.user)
            // let isAdmin = false;
            // if (userData.user === 'portaladmin') {
            //     isAdmin = true;
            // }
    ...
    const handleRouteSelect = (item) => (event) => {
        let selectedValue = item.value;
        let isChecked = event.target.checked;
        const { updatedValuesFromLayer, selectedValues } = updateRailways(selectedValue, isChecked, routeNames, railwayRoute)
        setRouteNames(updatedValuesFromLayer);
        setRailwayRoute(selectedValues)
        setRailwaySection([])
    };
    ...
    // const handleUserLoggingIn = async () => {
    //     
    //     if (userName.trim() === "" || userPassword.trim() === "") return toast.error("Please Enter Username and Password")
    //     let username = userName.trim();
    //     let password = userPassword.trim();
    //     try {
    //         let response = await fetch(API_BASE_URL +'/cris_login', {
    //             method: 'POST',
    //             headers: {
    //                 'Content-Type': 'application/json',
    //             },
    ...
    //         return toast.error('One or more password field is empty')
    //     }
    //     let oldpassword = oldPassword.trim();
    //     let newpassword = newPassword.trim();
    //     let confirmpassword = confirmNewPassword.trim();
    //     if(newpassword !== confirmpassword){
    //         return toast.error('New password not matched')
    //     }
    //     let username = userData?.username;
    //     try {
    //         let response = await fetch(API_BASE_URL +'/changePassword', {
    //             method: 'POST',
    //             headers: {
    //                 'Content-Type': 'application/json',
    //             },
    ...
    return (
        <>
            ...
            {/* {
                loginModal && isUserLoggedIn === false &&
                <Modal
                    open={loginModal}
                    sx={{
                        display: "flex",
                ...
                >
                    <LoginModal
                        ...
                        handleUserLoggingIn={handleUserLoggingIn}
            ...
            }
            {
                ...
                >
                    <ChangePasswordModal
                        ...
                        setConnfirmNewPassword={setConnfirmNewPassword}
                                ...
                                <div className="splitScreen-Header-Two-FilterDropdown">
                                                                            ...
                                                                            railwayNames && railwayNames.map((item, index) => (
                                                                                ...
                                                                                >
                                                                                    <input
                                                                                        type="checkbox"
                                                                                        value={item.value}
                                                                                        onChange={handleSelectedZoneLayer(item)}
                                    ...
                                    </svg>
                                    {
                                        ...
                                        <div className="userLoginDiv">
                                            ...
                                            {/* <div className="userDiv">
                                                <div className="userZone_Division">
                                                    {
                                                        userData && userData.division ? (
                                                            <>
                                                                <span>Zone : {userData.zone}</span>
                                                                <span>Division : {userData.division}</span>
                                                            </>
                                                        ) : (
                                                            <>
                                                                {
                                                                    userData.zone === "" && userData.division === "" ? (
                                                                        <>
                                                                            <span>Zone : All</span>
                                                                            <span>Division : All</span>
                                                                        </>
                                                                    ) : (
                                                                        <>
                                                                            <span>Zone : {userData.zone}</span>
